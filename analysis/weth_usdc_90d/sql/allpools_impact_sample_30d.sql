-- FULL-MARKET price-impact sample for STRICTLY-RETAIL WETH/USDC transactions —
-- the impact curve of the ENTIRE current on-chain WETH/USDC market across ALL
-- venues (Uniswap V3+V4+V2, Fluid, Swaap, Pancake, Balancer, Curve, …), built
-- entirely from uniswap-labs.research.markout_prod.
--
-- Why all venues (not just V3): the §4 calibration referenced spread vs each
-- pool's own pre-trade mid, which needed V3 sqrtPriceX96 — hence V3-only. We have
-- since moved the calibration target to the LAGGED BINANCE MID, which needs only
-- (a) the routed order amounts (execution price = ΣUSDC/ΣWETH, and side) and
-- (b) the fair grid. Neither needs a per-pool mid, so there is no reason to
-- restrict to V3. markout_prod carries token amounts + the Binance benchmark for
-- every venue, so the whole sample comes from one table.
--
-- Why full-market: to test a NEW strategy it becomes the submission pool and the
-- 5bp pool rejoins the market it competes against. So the normalizer must
-- represent ALL pools currently on-chain on the pair. Fitting a single V2 to this
-- curve gives the "full-market normalizer" (φ, depth) — the competitor a new
-- mechanism faces. (The §4 non-5bp sample, used to validate the methodology
-- against the held-out 5bp pool, is kept separately.)
--
-- Cohort (same as §4): strict retail — Uniswap first-party FE (core.swaps surface
-- tag) ∪ MetaMask 87.5bps-fee. Calibration target: observed_spread_fair_lag_bps
-- (spread vs the Binance mid one 12s step before the trade). observed_spread_fair_bps
-- (contemporaneous fair) kept as a diagnostic. No pool-mid column (not needed; would
-- require per-venue mid reconstruction). Window: 30d, 2026-04-21..2026-05-20.
...
-- units. This single CTE feeds both the fair grid (all legs) and the calibration
-- legs (strict-cohort subset).
mk_all AS (
  SELECT
...
-- 12s Binance-mid grid (from ALL WETH/USDC benchmarks), forward-filled.
fair_bucket_obs AS (
  SELECT
...
-- Calibration legs: strict-retail cohort, inside the window, valid amounts.
legs AS (
  SELECT m.*
  FROM mk_all AS m
