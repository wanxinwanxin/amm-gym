"""Cycle 2 — Joint Nelder-Mead minimizing combined calibration+holdout loss.

The DONE criterion requires <2% residual on BOTH calibration AND holdout seed
sets. Cycle 1 found a perfect calibration fit but holdout T1 is off by 26%
due to the model's high seed-to-seed variance (bimodal arb routing).

Strategy: include both seed sets in the loss so the optimizer finds a point
that balances both averages.
"""
...
CYCLE = 2
JOINT_SEEDS = tuple(CALIB_SEEDS) + tuple(HOLDOUT_SEEDS)  # 10 seeds
...
def joint_loss_factory(log: list[dict]):
    """Loss = sum of residuals^2 on BOTH calibration & holdout means."""
    def loss(x: np.ndarray) -> float:
        ...
        entry = {
            ...
            "calib": {**{k: float(rc[k]) for k in rc}, "T1": mc["arb_5bp_share"],
                      "T2": mc["retail_5bp_share"], "T3": mc["markout_bps"]},
            "holdout": {**{k: float(rh[k]) for k in rh}, "T1": mh["arb_5bp_share"],
                        "T2": mh["retail_5bp_share"], "T3": mh["markout_bps"]},
            "loss": L,
...
if __name__ == "__main__":
    # Seed from Cycle 1's best
    init = Params(
        submission_depth_y=103448571.128,
        normalizer_fee=0.04787528,
        normalizer_depth_y=114152340566.816,
    )
    print("Cycle 2 — joint calibration+holdout Nelder-Mead")
    ...
    max_rc = max(abs(v) for v in rc.values())
    max_rh = max(abs(v) for v in rh.values())
    print(f"max|rc|={max_rc:.4f}  max|rh|={max_rh:.4f}  DONE={max_rc < 0.02 and max_rh < 0.02}")
