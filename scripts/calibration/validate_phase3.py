"""Phase 3 — Validate the V2 impact-curve fit by running the simulator with:
  - 5bp pool (submission) frozen at real on-chain params: fee=0.0005, depth=$212.16M
  - "Other pool" (normalizer) at the Phase 2 fitted (φ, depth)

Compute predicted 5bp metrics across seeds, compare to real-world held-out
targets (which were NEVER used as optimization inputs in the new framework).

Outputs:
  analysis/weth_usdc_90d/calibration_v2_final.json
  reports/calibration_v2_validation.md
  plots/validation_sim_vs_real.png
"""
...
FIT_JSON = ANALYSIS_DIR / "impact_curve_fit.json"
OUT_JSON = ANALYSIS_DIR / "calibration_v2_final.json"
REPORT_PATH = REPO_ROOT / "reports" / "calibration_v2_validation.md"
PLOT_PATH = REPO_ROOT / "plots" / "validation_sim_vs_real.png"
