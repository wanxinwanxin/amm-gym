"""Benchmark fixed-fee and greedy clairvoyant controllers on challenge tapes."""
...
def _config_factory(n_steps: int | None, submission_liquidity_fraction: float):
    if n_steps is None and submission_liquidity_fraction == 1.0:
        return None

    def factory(seed: int) -> ExactSimpleAMMConfig:
        config = ExactSimpleAMMConfig.from_seed(seed)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run clairvoyant challenge-tape oracle benchmarks")
    ...
    parser.add_argument("--controller", choices=("greedy_step", "structured_retail"), default="greedy_step")
    parser.add_argument("--fee-grid-size", type=int, default=41)
    ...
    baseline = run_clairvoyant_batch(
